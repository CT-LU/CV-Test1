			...
			for (int d = minDisparity; d <= maxDisparity; d++)
			{
				...
				for (int i = -RAD; i <= RAD; i++)
					for (int j = -RAD; j <= RAD; j++)
					{
						...
						if (xxd >= w) xxd = w - 1;
						// We got coordinates of the RAD since here
				...
				{
					bestCost = cost;
					bestDisparity = d * 7; // d is 0 ~ 32 , make it bigger
				}
...
{
	// Search ranges is from -32 to 0, that means each pixel value of a depth image is 0 ~ 32 
	int minDisp = -32; 
	int maxDisp = 0;
